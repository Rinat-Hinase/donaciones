...
export default function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const data = await listDonations({ campanaId, max: 500 });
        setRows(data);
      } catch (e) {
  ...
  const top3 = useMemo(() => {
    const map = new Map();
    for (const r of rows) {
      const key = (r.donante_nombre_lower || (r.donante_nombre || "").toLowerCase() || "anónimo").trim() || "anónimo";
      const display = r.donante_nombre || "Anónimo";
      const prev = map.get(key) || { nombre: display, total: 0, count: 0 };
      map.set(key, { nombre: display, total: prev.total + (Number(r.monto) || 0), count: prev.count + 1 });
    }
        ...
        >
          ...
          >
            ...
            <div className={kpi}>
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-50">{fmt.format(promedio)}</h2>
            </div>
          </motion.section>
          ...
          >
            ...
            {loading ? (
              ...
              <p className="text-sm text-slate-500">
                Aún no hay donaciones. Toca el botón “+” para agregar la primera.
              </p>
            ) : (
